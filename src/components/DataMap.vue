  ...
  setup() {
    ...
    const initMap = () => {
      ...
      map.on("load", function () {
        ...
        map.addSource("colleges", { type: "geojson", data: collegesLocalized });
      ...
      map.on("mouseenter", "colleges", (e:any) => {
        ...
        const html = `<div class="map-popup-content" style="color:black;"><b>${name}</b><br>${division}<br>Within ${hoursAway}hrs</div>`;
    ...
    const getData = async () => {
      let result = await fetch("/assets/data/colleges_with_distance.geojson");
      collegesLocalized = await result.json();
